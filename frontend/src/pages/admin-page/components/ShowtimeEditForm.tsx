...
function ShowtimeEditForm({ showtime, onSuccess, onCancel }: ShowtimeEditFormProps) {
      ...
      try {
        ...
        if ((!showtime.tierPrices || showtime.tierPrices.length === 0) && hallsData.length > 0) {
          ...
          if (currentHall?.seatTiers) {
            setTierPrices(
              currentHall.seatTiers.map((t) => ({ tierName: t.tierName, price: 1500.0 }))
            );
          }
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to update showtime");
      addNotification({
        type: 'error',
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-zinc-950/80 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white uppercase tracking-tight">Edit Scheduled Showtime</h3>
              <p className="text-[11px] text-zinc-400">Modify screening date, hall or ticket prices</p>
            </div>
          </div>
          <button
            type="button"
        ...
        {loadingData ? (
          <div className="p-12 text-center text-zinc-400 flex flex-col items-center justify-center gap-3 flex-1">
            ...
            <span className="text-xs">Loading scheduling data...</span>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="p-4 bg-zinc-950/80 border-t border-white/10 flex items-center justify-end gap-3 shrink-0">
              ...
              >
                {submitting ? "Saving Changes..." : "Update Showtime"}
              </button>
            </div>
          </form>
        )}
