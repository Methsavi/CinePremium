      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-zinc-950/80 border-b border-white/10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white uppercase tracking-tight">Add New Movie</h3>
              <p className="text-[11px] text-zinc-400">Provide title, media and screening details</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
          <div className="p-6 sm:p-7 space-y-5 overflow-y-auto flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase mb-1.5">
                  Release Date (or display text)
                </label>
                <input
                  type="text"
                  value={releaseDate}
                  onChange={(e) => setReleaseDate(e.target.value)}
                  placeholder="e.g. Nov 24, 2026 or Coming Soon"
                  className="w-full bg-zinc-950 border border-white/10 text-white text-sm rounded-xl px-3.5 py-2.5 focus:outline-none focus:border-red-500 transition-colors"
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold text-zinc-400 uppercase mb-1.5">
                Trailer YouTube Embed Link (Optional)
              </label>
              <input
                type="text"
                value={trailerUrl}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <div>
                ...
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 hover:border-red-500/50 rounded-2xl p-4 cursor-pointer bg-zinc-950 transition-colors">
                  ...
                  <span className="text-xs text-zinc-300 font-medium">Click to upload poster</span>
                  <span className="text-[10px] text-zinc-500">PNG, JPG or WebP</span>
                  <input
                    type="file"
                    accept="image/*"
                ...
                {posterPreview && (
                  <div className="mt-2 flex items-center gap-2 p-2 bg-zinc-950 rounded-xl border border-white/10">
                    ...
                    <span className="text-xs text-zinc-300 truncate">{posterFile?.name}</span>
                  </div>
                )}
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-400 uppercase mb-1.5">
                  Horizontal Backdrop Hero
                </label>
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 hover:border-red-500/50 rounded-2xl p-4 cursor-pointer bg-zinc-950 transition-colors">
                  <Upload className="w-6 h-6 text-zinc-500 mb-1" />
                  <span className="text-xs text-zinc-300 font-medium">Click to upload backdrop</span>
                  <span className="text-[10px] text-zinc-500">16:9 Landscape recommended</span>
                  <input
                    type="file"
                    accept="image/*"
                ...
                {backdropPreview && (
                  <div className="mt-2 flex items-center gap-2 p-2 bg-zinc-950 rounded-xl border border-white/10">
                    ...
                    <span className="text-xs text-zinc-300 truncate">{backdropFile?.name}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="p-4 bg-zinc-950/80 border-t border-white/10 flex items-center justify-end gap-3 shrink-0">
            ...
            >
              {submitting ? "Uploading & Saving..." : "Save Movie to Database"}
            </button>
          </div>
        </form>
      </div>
